...
object DeviceManager {
  ...
  final case class RequestTrackDevice(groupId: String, deviceId: String, replyTo: ActorRef[DeviceRegistered]) extends DeviceManagerMessage with DeviceGroupMessage
  ...
  final case class RequestAllTemperatures(requestId: Long, groupId: String, replyTo: ActorRef[RespondAllTemperatures]) extends DeviceGroupQueryMessage with DeviceGroupMessage with DeviceManagerMessage
...
class DeviceManager(context: ActorContext[DeviceManager.DeviceManagerMessage])
  ...
  override def onMessage(msg: DeviceManagerMessage): Behavior[DeviceManagerMessage] =
    msg match {
        // TODO vragen wat trackMsgt @ is
        // Device manager krijgt een message ,et group id en device id, gaat kijken als die groupid al bestaat, zo niet gaat hij hem aanmaken en toevoegen aan de state
        // registering a device group
      case trackMsg @ RequestTrackDevice(groupId, _, replyTo) ⇒
        groupIdToActor.get(groupId) match {
          ...
          case None ⇒
            ...
            // death watcher, wanneer een groupactor voor een of andere reden terminated is moet hij die message uitsturen
        ...
        // vraagt de device list op , wanneer een device list te vinden is dan geeft hij de lijst terug anders een lege lijst
        // oplijsten actieve device IDs
      case req @ RequestDeviceList(requestId, groupId, replyTo) ⇒
        ...
        // vraagt alle temperaturen op va een device group
